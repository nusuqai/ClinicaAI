...
export default function DashboardAppointments() {
  ...
  const AppointmentCard = ({ apt, isPast }) => {
    ...
    const doctorAvatar = apt.doctors?.photo_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(apt.doctors?.full_name || 'طبيب')}&background=00C2CB&color=fff`;

    return (
      <div className={`bg-white rounded-3xl p-6 shadow-sm border ${isPast ? 'border-primary/5 opacity-80' : 'border-accent/20'} flex flex-col md:flex-row gap-6 items-start md:items-center justify-between transition-all hover:shadow-md`}>
        
        {/* Doctor Info */}
        <div className="flex items-center gap-4 w-full md:w-auto">
          <img src={doctorAvatar} alt={apt.doctors?.full_name} className="w-16 h-16 rounded-2xl object-cover shrink-0" />
          <div>
            <h3 className="font-heading font-bold text-lg text-primary">{apt.doctors?.full_name}</h3>
            <p className="font-sans text-sm text-text/60">{apt.doctors?.specialty}</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between md:justify-end gap-4 w-full md:w-auto">
          <span className={`px-4 py-1.5 rounded-full font-sans text-xs font-bold ${status.color}`}>
            {status.label}
          </span>
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        {[1,2,3].map(i => <div key={i} className="animate-pulse bg-white/50 h-32 rounded-3xl border border-primary/5" />)}
      </div>
    )
  }
  ...
  return (
    <div className="space-y-10 animate-in fade-in zoom-in-95 duration-500">
      <header>
        <h1 className="text-3xl font-heading font-bold text-primary mb-2">مواعيدي</h1>
        <p className="text-text/60 font-sans">إدارة جميع حجوزاتك ومواعيدك الطبية.</p>
      </header>
      ...
      <section>
        ...
        {upcoming.length > 0 ? (
          ...
          <div className="bg-white/50 rounded-3xl p-10 text-center border border-primary/5">
             ...
             <p className="font-sans text-text/60">عظيم! تم الانتهاء من جميع مواعيدك. لا يوجد مواعيد قادمة.</p>
          </div>
        )}
      </section>
      ...
      {/* Past */}
      <section>
        <h2 className="font-heading font-bold text-xl text-primary/70 mb-4 border-t border-primary/10 pt-8">المواعيد السابقة</h2>
        {past.length > 0 ? (
          <div className="space-y-4">
